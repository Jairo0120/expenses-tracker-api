...
from ..models import Saving, Cycle, RecurrentSaving
...
def savings_fixture(session: Session, cycles):
    saving_1 = Saving(id=1, description="Saving 1", val_saving=100, cycle_id=1)
    saving_2 = Saving(id=2, description="Saving 2", val_saving=200, cycle_id=1)
    saving_3 = Saving(id=3, description="Saving 3", val_saving=300, cycle_id=2)
    saving_4 = Saving(id=4, description="Saving 4", val_saving=300, cycle_id=3)
...
def test_list_savings(client: TestClient, savings):
    ...
    assert response.json()[0]["description"] == "Saving 2"
...
def test_list_savings_different_cycle(client: TestClient, savings):
    ...
    assert response.json()[0]["description"] == "Saving 3"
...
def test_create_saving_no_cycle(client: TestClient, cycles):
    ...
    assert response.json()["description"] == "Saving 4"
...
def test_create_saving_with_cycle(client: TestClient, cycles):
    ...
    assert response.status_code == 201
    assert response.json()["description"] == "Saving 5"
...
def test_create_saving_with_recurrent_saving(
    ...
    assert response.status_code == 201
    assert response.json()["description"] == "Saving 7"
    assert response.json()["is_recurrent_saving"] is True
    assert len(recurrent_saving) == 1
    assert recurrent_saving[0].description == "Saving 7"
...
def test_update_saving(client: TestClient, savings, cycles):
    ...
    assert response.status_code == 200
    assert response.json()["description"] == "Saving 1 updated"
