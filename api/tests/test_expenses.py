...
from ..models import Expense, Cycle, Budget, RecurrentExpense
...
def budget_fixtures(session: Session, cycles):
    ...
    budget_2 = Budget(id=2, description="Budget 2", val_budget=200, cycle_id=1)
    budget_3 = Budget(id=3, description="Budget 3", val_budget=300, cycle_id=2)
...
def test_create_expense(
    ...
    recurrent_expenses = session.exec(
        select(RecurrentExpense).where(
            RecurrentExpense.description == "Expense 4"
        )
    ).first()
    data = response.json()
    ...
    assert recurrent_expenses is None


def test_create_recurrent_expense(
    client: TestClient, session, expenses, cycles
):
    req_data = {
        "description": "Expense 4",
        "val_expense": 400,
...
def test_update_cycle_budget_should_empty(
    client: TestClient, expenses, cycles, budgets
):
    req_data = {
...
def test_update_budget_should_be_in_cycle(
    client: TestClient, expenses, cycles, budgets
):
    req_data = {
        "description": "Expense 2 Updated",
